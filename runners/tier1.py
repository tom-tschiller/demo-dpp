...
async def main(args):
    tier1_agent = await create_agent_with_args(args, ident="tier1")

    try:
        log_status(
            "#7 Provision an agent and wallet, get back configuration details"
            + (
                f" (Wallet type: {tier1_agent.wallet_type})"
                if tier1_agent.wallet_type
                else ""
            )
        )
        agent = Tier1Agent(
            "tier1.agent",
            tier1_agent.start_port,
            tier1_agent.start_port + 1,
            genesis_data=tier1_agent.genesis_txns,
            genesis_txn_list=tier1_agent.genesis_txn_list,
            no_auto=tier1_agent.no_auto,
            tails_server_base_url=tier1_agent.tails_server_base_url,
            revocation=tier1_agent.revocation,
            timing=tier1_agent.show_timing,
            multitenant=tier1_agent.multitenant,
        ...
        await tier1_agent.initialize(the_agent=agent)
        ...
        async for option in prompt_loop(options):
            ...
            if option is None or option in "xX":
            ...
            elif option in "dD" and tier1_agent.endorser_role:
                endorser_did = await prompt("Enter Endorser's DID: ")
                await tier1_agent.agent.admin_POST(
                    f"/transactions/{tier1_agent.agent.connection_id}/set-endorser-info",
                    params={"endorser_did": endorser_did, "endorser_name": "endorser"},
                )

            elif option in "wW" and tier1_agent.multitenant:
                target_wallet_name = await prompt("Enter wallet name: ")
                include_subwallet_webhook = await prompt(
                    "(Y/N) Create sub-wallet webhook target: "
                )
                if include_subwallet_webhook.lower() == "y":
                    await tier1_agent.agent.register_or_switch_wallet(
                        target_wallet_name,
                        webhook_port=tier1_agent.agent.get_new_webhook_port(),
                        mediator_agent=tier1_agent.mediator_agent,
                ...
                if msg:
                    await tier1_agent.agent.admin_POST(
                        f"/connections/{tier1_agent.agent.connection_id}/send-message",
                        {"content": msg},
                    )
        ...
        if tier1_agent.show_timing:
            ...
            if timing:
                for line in tier1_agent.agent.format_timing(timing):
                    log_msg(line)

    finally:
        terminated = await tier1_agent.terminate()
...
if __name__ == "__main__":
    parser = arg_parser(ident="tier1", port=8080)
